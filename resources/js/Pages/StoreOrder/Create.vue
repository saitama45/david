...
onBeforeMount(() => {
    const previousData = localStorage.getItem("storeOrderCreateDraft");
    if (previousData) {
...
onMounted(() => {
    if (drafts.value) {
        confirm.require({
            message: "You have an unfinished draft. Would you like to continue where you left off or discard the draft?",
            ...
            accept: () => {
                ...
                // Fetch items and dates for the draft's supplier
                if(orderForm.supplier_id) {
                    fetchSupplierItems(orderForm.supplier_id);
                    // Manually trigger date and branch fetches if data exists
            ...
            reject: () => {
                localStorage.removeItem("storeOrderCreateDraft");
            }
        });
    }
...
watch(orderForm, (value) => {
    if (isMountedAndReady.value) {
        localStorage.setItem("storeOrderCreateDraft", JSON.stringify(value));
    }
}, { deep: true });
...
const productDetails = reactive({
    ...
    base_uom: null, base_qty: null, quantity: null, cost: null, total_cost: null
});
...
const addToOrdersButton = () => {
    ...
    if (existingItemIndex !== -1) {
        ...
        orderForm.orders.push({ ...productDetails, quantity: currentQuantity, base_uom_qty, total_cost });
    }
    ...
    confirm.require({
        ...
        accept: () => {
            orderForm.post(route("store-orders.store"), {
                ...
                onError: (e) => toast.add({ severity: "error", summary: "Error", detail: e.error || e.message || "Can't place the order.", life: 5000 }),
            });
        }
    });
};
...
watch(productId, async (itemCode) => {
    if (itemCode) {
        ...
        try {
            ...
            if (result) {
                productDetails.id = result.ItemCode;
...
watch(() => orderForm.supplier_id, async (supplierCode) => {
    if (!isMountedAndReady.value && !drafts.value) return;
...
watch(() => orderForm.order_date, async (newDate) => {
    ...
    if (newDate && orderForm.supplier_id) {
        try {
            ...
            toast.add({ severity: "error", summary: "Error", detail: "Failed to load store branches for the selected date.", life: 5000 });
        }
    } else {
        dynamicBranches.value = {};
    }
...
const areOrderDetailsLocked = computed(() => orderForm.orders.length > 0);
                ...
                <Card>
                    ...
                    <CardContent class="space-y-3">
                        <div class="flex flex-col space-y-1">
                            ...
                            <Select filter placeholder="Select a Supplier" v-model="orderForm.supplier_id" :options="suppliersOptions" optionLabel="label" optionValue="value" :disabled="areOrderDetailsLocked" />
                        ...
                        <div class="flex flex-col space-y-1">
                            ...
                            <Select filter placeholder="Select a Store" v-model="orderForm.branch_id" :options="branchesOptions" optionLabel="label" optionValue="value" :disabled="areOrderDetailsLocked" />
