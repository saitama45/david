...
let filter = ref(usePage().props.filters.currentFilter || "approved_lvl1");
let search = ref(usePage().props.filters.search);

watch(filter, function (value) {
    router.get(
        route("wastage-approval-lvl2.index"),
...
const showWastageDetails = (id) => {
    router.get(`/wastage-approval-level2/show/${id}`);
};
...
<template>
    <Layout
        heading="Wastage Approval 2nd Level"
    ...
    >
        <FilterTab>
            ...
            <Button
                class="sm:px-10 px-3 bg-white/10 text-gray-800 hover:text-white gap-5 sm:text-sm text-xs"
                :class="isFilterActive('approved_lvl1')"
                @click="changeFilter('approved_lvl1')"
            >APPROVED LEVEL 1
                <Badge
                    class="sm:flex hidden border border-gray bg-transparent text-gray-900 px-2"
                    :class="isFilterActive('approved_lvl1')"
                >{{ counts.approved_lvl1 }}</Badge>
            </Button>
        ...
        <TableContainer>
            ...
            <Table>
                <TableHead>
                    ...
                    <TH>Total Qty</TH>
                    <TH>Items</TH>
                    <TH v-if="hasAccess('view total cost in wastage approval level 2')">Total Cost</TH>
                    <TH>Status</TH>
                    <TH>Date</TH>
                    <TH>Actions</TH>
                </TableHead>
                <TableBody>
                    <tr v-for="wastage in wastages.data" :key="wastage.id">
                        ...
                        <TD v-if="hasAccess('view total cost in wastage approval level 2')">{{ formatCurrency(wastage.total_cost) }}</TD>
                        <TD :class="hasAccess('view total cost in wastage approval level 2') ? '' : 'w-32'">
                            <Badge
                                :class="statusBadgeColor(wastage.wastage_status)"
                                class="font-bold"
                            >{{ wastage.wastage_status?.toUpperCase().replace('_', ' ') ?? "N/A" }}</Badge>
                        </TD>
                        <TD v-if="hasAccess('view total cost in wastage approval level 2')">{{ formatDate(wastage.created_at) }}</TD>
                        <TD v-if="hasAccess('view total cost in wastage approval level 2')" class="flex">
                            <Button
            ...
            <MobileTableContainer>
                <MobileTableRow v-for="wastage in wastages.data" :key="wastage.id">
                    ...
                    >
                    <LabelXS
                        v-if="hasAccess('view total cost in wastage approval level 2')"
