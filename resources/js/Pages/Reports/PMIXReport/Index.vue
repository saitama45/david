...
const updateFilters = () => {
    ...
    router.get(
        route('reports.pmix-report.index'),
        {
...
const exportRoute = computed(() =>
    route('reports.pmix-report.export', {
        date_from: dateFrom.value,
        date_to: dateTo.value,
        store_ids: storeIds.value,
        search: search.value,
...
// Dynamic store columns for the table
const dynamicStoreColumns = computed(() => {
    return Object.entries(props.storeColumns).map(([storeId, storeName]) => ({
        id: storeId,
        name: storeName
    }));
});
...
<template>
    <Layout heading="PMIX Report" :hasExcelDownload="true" :exportRoute="exportRoute">
                ...
                <div class="mb-6">
                    <div class="relative">
                        ...
                        <Input
                            ...
                            placeholder="Search by POS code, description, category, or sub-category..."
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ formatNumber(reportTotals.quantity) }}</p>
                    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table class="min-w-full">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr v-for="(item, index) in (paginatedData.data || [])" :key="item.POSCode" class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-sm font-mono text-gray-900">{{ item.POSCode || 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-900 max-w-xs truncate" :title="item.POSDescription">{{ item.POSDescription || 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ item.Category || 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ item.SubCategory || 'N/A' }}</td>

                            <!-- Dynamic Store Data -->
                            <template v-for="store in dynamicStoreColumns" :key="'data-' + store.id">
                                <td class="px-3 py-4 text-sm text-center font-medium text-gray-900">
                                    {{ formatNumber(item.stores[store.id]?.quantity || 0) }}
                                </td>
            ...
            <div class="lg:hidden">
                ...
                <div class="divide-y divide-gray-100">
                    <div v-for="(item, index) in (paginatedData.data || [])" :key="item.POSCode" class="p-4 hover:bg-gray-50 transition-colors">
                        <!-- Header Row -->
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex flex-col">
                                <span class="text-sm font-mono text-gray-900">{{ item.POSCode || 'N/A' }}</span>
                                <span class="text-xs text-gray-500">{{ item.Category || 'N/A' }}</span>
                            </div>
                        ...
                        <!-- Description -->
                        <div class="mb-3">
                            <p class="text-sm text-gray-900 line-clamp-2">{{ item.POSDescription || 'N/A' }}</p>
                        </div>

                        <!-- Store Breakdown -->
                        <div class="space-y-2">
                            <div v-for="store in dynamicStoreColumns" :key="'mobile-' + store.id" class="flex items-center justify-between text-sm">
