        ...
        <div class="mt-8">
            ...
            <TableContainer>
                ...
                <div class="hidden md:block">
                    <Table>
                        <TableHead>
                            ...
                            <TH v-if="can.view_transaction">Actions</TH>
                        </TableHead>
                        <TableBody>
                            <tr v-if="!transactions.data.length">
                                <td :colspan="can.view_transaction ? 7 : 6" class="text-center py-4">No transactions found.</td>
                            </tr>
                            <tr v-for="transaction in transactions.data" :key="`${transaction.schedule_id}-${transaction.branch_id}`">
                                ...
                                <TD v-if="can.view_transaction">
                                    <Button @click="viewReviewPage(transaction.schedule_id, transaction.branch_id)" variant="outline" size="icon">
                                        <Eye class="h-4 w-4" />
                                    </Button>
                                </TD>
                            </tr>
                ...
                <div class="md:hidden grid grid-cols-1 gap-4">
                    <div v-if="!transactions.data.length" class="text-center py-4 text-gray-500">
                        No transactions found.
                    </div>
                    <div v-for="transaction in transactions.data" :key="`${transaction.schedule_id}-${transaction.branch_id}`" class="bg-white p-4 rounded-lg shadow">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="font-semibold text-gray-800">{{ transaction.branch_name }}</p>
                                <p class="text-sm text-gray-600">{{ getMonthName(transaction.month) }} {{ transaction.year }}</p>
                                <p class="text-xs text-gray-500">Date: {{ formatDate(transaction.calculated_date) }}</p>
                            </div>
                            <Button v-if="can.view_transaction" @click="viewReviewPage(transaction.schedule_id, transaction.branch_id)" variant="outline" size="icon">
                                <Eye class="h-5 w-5" />
                            </Button>
                        </div>
                        <div class="mt-4 pt-4 border-t">
                            <div class="text-sm mb-2">
                                ...
                                <span class="font-medium text-gray-800">{{ transaction.uploader_name }}</span>
                            </div>
                            <div class="text-sm">
