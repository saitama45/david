...
function R(e, t2, n = /* @__PURE__ */ new WeakSet()) {
  ...
  n.add(e).add(t2);
  let r = Array.isArray(e), o = Array.isArray(t2), u, f, h;
  if (r && o) {
    if (f = e.length, f != t2.length) return false;
    for (u = f; u-- !== 0; ) if (!R(e[u], t2[u], n)) return false;
    return true;
  }
  if (r != o) return false;
  ...
  let O = Object.keys(e);
  if (f = O.length, f !== Object.keys(t2).length) return false;
  for (u = f; u-- !== 0; ) if (!Object.prototype.hasOwnProperty.call(t2, O[u])) return false;
  for (u = f; u-- !== 0; ) if (h = O[u], !R(e[h], t2[h], n)) return false;
  return true;
}
function y2(e, t2) {
  return R(e, t2);
...
function k(e, t2, n) {
  return n ? c2(e, n) === c2(t2, n) : y2(e, t2);
}
...
function Y2(e, t2 = "", r = "", s4 = [], o) {
  if (p(e)) {
    ...
    if (dt(a2)) return;
    if (z2(a2, v2)) {
      let n = a2.replaceAll(v2, (l2) => {
        let c3 = l2.replace(/{|}/g, "").split(".").filter((m2) => !s4.some((u) => z2(m2, u)));
...
var le = (e = {}, t2, r, s4) => {
  if (t2) {
    let { variable: o, options: a2 } = g3.defaults || {}, { prefix: n, transform: l2 } = (e == null ? void 0 : e.options) || a2 || {}, i2 = z2(t2, v2) ? t2 : `{${t2}}`;
...
function ce(e, t2 = {}) {
  ...
  for (; i2.length; ) {
    ...
    for (let p2 in m2) {
      let h = m2[p2], y3 = ke(h), x2 = z2(p2, a2) ? Q(u) : Q(u, ee(p2));
      if (i(y3)) i2.push({ node: y3, path: x2 });
      else {
        let k2 = ne(x2), w2 = Y2(y3, x2, s4, [a2]);
        _e(l2, k2, w2);
        let $2 = x2;
...
var S = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  if (s2(t2) && r.transform !== "strict") {
    let k2 = e.replace("-directive", ""), m2 = t2, { colorScheme: w2, extend: $2, css: j } = m2, V = b2(m2, ["colorScheme", "extend", "css"]), u = $2 || {}, { colorScheme: D } = u, z3 = b2(u, ["colorScheme"]), p2 = w2 || {}, { dark: E } = p2, L = b2(p2, ["dark"]), h = D || {}, { dark: te } = h, re = b2(h, ["dark"]), K2 = s2(V) ? this._toVariables({ [k2]: d2(d2({}, V), z3) }, r) : {}, M2 = s2(L) ? this._toVariables({ [k2]: d2(d2({}, L), re) }, r) : {}, X = s2(E) ? this._toVariables({ [k2]: d2(d2({}, E), te) }, r) : {}, [B, G2] = [(y3 = K2.declarations) != null ? y3 : "", K2.tokens || []], [I, U2] = [(N2 = M2.declarations) != null ? N2 : "", M2.tokens || []], [H, W] = [(x2 = X.declarations) != null ? x2 : "", X.tokens || []], q2 = this.transformCSS(k2, `${B}${I}`, "light", "variable", r, o, a2, n), Z = this.transformCSS(k2, H, "dark", "variable", r, o, a2, n);
...
//# sourceMappingURL=chunk-CGPNUB4N.js.map
