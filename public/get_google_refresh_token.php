...
// Required scope for Google Drive and to test the connection
define('SCOPE', 'https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/userinfo.email');

// Start session to store state
session_start();

// If tokens are already in the session, we can show the success state directly.
if (isset($_SESSION['tokens']['refresh_token'])) {
    $success = true;
    $refreshToken = $_SESSION['tokens']['refresh_token'];
    $accessToken = $_SESSION['tokens']['access_token'];
}
...
/**
 * Finds or downloads a CA bundle.
 *
 * @return string|null Path to the CA bundle, or null if not found.
 */
function find_or_download_ca_bundle() {
    // Path for the local CA bundle
    $local_ca_path = __DIR__ . '/cacert.pem';

    // If we have a local, recent CA bundle, use it.
    if (file_exists($local_ca_path) && (time() - filemtime($local_ca_path) < 60 * 24 * 3600)) {
        return $local_ca_path;
    }

    // Check php.ini for a configured CA bundle
    $ini_ca_path = ini_get('curl.cainfo') ?: ini_get('openssl.cafile');
    if ($ini_ca_path && file_exists($ini_ca_path)) {
        return $ini_ca_path;
    }

    // If no CA bundle is found or is outdated, try to download a fresh one.
    // This is a potential security risk as we disable peer verification for this one download.
    ...
    $cacert_content = @file_get_contents($cacert_url, false, $context);

    if ($cacert_content) {
        @file_put_contents($local_ca_path, $cacert_content);
        if (file_exists($local_ca_path)) {
            return $local_ca_path;
        }
    }

    // If download fails but we have an old local copy, use it as a last resort.
    if (file_exists($local_ca_path)) {
        return $local_ca_path;
    }
    
    // If download fails but we had an ini path, return it.
    if ($ini_ca_path) {
        return $ini_ca_path;
    }

    return null; // Unable to find or download a CA bundle.
}
...
function makeRequestWithCurl($url, $data = null, $method = 'GET', $headers = []) {
    ...
    // Find and set the CA bundle path
    $ca_path = find_or_download_ca_bundle();
    if ($ca_path) {
    ...
    if (curl_error($ch)) {
        ...
        curl_close($ch);
        // Provide a more helpful error message for SSL issues
        if (strpos($error_message, 'SSL certificate') !== false) {
            throw new Exception('cURL Error: ' . $error_message . '. The script tried to automatically download a CA certificate bundle but failed. Please manually download `cacert.pem` from https://curl.se/ca/cacert.pem and place it in the same directory as this script (`' . __DIR__ . '`).');
        }
...
function makeRequestWithFileGetContents($url, $data = null, $method = 'GET', $headers = []) {
    ...
    // Explicitly check for a cafile in php.ini to avoid issues on some PHP versions
    // where `cafile => null` can cause errors.
    ...
    if ($response === false) {
        ...
        if (strpos($errorMessage, 'SSL') !== false || strpos($errorMessage, 'certificate') !== false) {
            throw new Exception('SSL Connection Failed: ' . $errorMessage . '. This is often due to an outdated or misconfigured CA certificate bundle. Please ensure your `openssl.cafile` setting in `php.ini` points to a valid `cacert.pem` file. Enabling the `php-curl` extension is also highly recommended.');
...
if (isset($_GET['code'])) {
    // If tokens are already in the session, it means this is a refresh. Redirect to clean URL.
    ...
    if (!isset($_SESSION['oauth_state']) || $_GET['state'] !== $_SESSION['oauth_state']) {
        ...
        try {
            ...
            $result = makeRequest(TOKEN_URL, $data, 'POST');

            if ($result['http_code'] === 200) {
                $tokens = json_decode($result['response'], true);

                if (isset($tokens['refresh_token'])) {
                    // Store tokens and clear state
                    $_SESSION['tokens'] = $tokens;
                    unset($_SESSION['oauth_state']);

                    // Redirect to a clean URL to prevent re-processing and enable refresh.
                    header('Location: ' . REDIRECT_URI);
                    exit;
                } else {
...
if (isset($_POST['test_connection']) && isset($_SESSION['tokens']['access_token'])) {
    try {
        ...
        // Use the robust makeRequest function to handle cURL/SSL correctly
        $result = makeRequest($testUrl, null, 'GET', $headers);
        ...
        if ($result['http_code'] === 200) {
            ...
            $testError = 'Failed to connect. HTTP Status: ' . $result['http_code'] . ', Response: ' . $result['response'];
        }
    } catch (Exception $e) {
    ...
    <div class="container">
        ...
        <?php if (isset($success)): ?>
            <div class="success">
                ...
                <p><a href="?start_over=1" class="btn btn-secondary">🔄 Start Over & Clear Session</a></p>
            </div>
        <?php else: ?>
